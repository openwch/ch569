...
* Version            : V1.1
* Date               : 2021/07/31
* Description 		 : CH565/569 TCP/IP客户端连接服务器演示例程，使用以太网时，主频不得低于60MHz
...
/* TCP/IP协议栈本地参数 */
UINT8 local_mac[6]={0x84,0xc2,0xe4,0x01,0x02,0x03,};            /* 本地MAC地址，会在后面填入实际的MAC地址  */
...
UINT8 CH569NET_LibInit(UINT8 *ip, UINT8 *gwip, UINT8 *mask, UINT8 *macaddr)
{
    ...
    if(CH569NET_GetVer() != CH569NET_LIB_VER)return 0xfc;                     /* 获取库的版本号，检查是否和头文件一致 */
...
void CH569NET_HandleGlobalInt(void)
{
    ...
    if(initstat & GINT_STAT_SOCKET)                                              /* Socket中断 */
    {
        for(i = 0; i < CH569NET_MAX_SOCKET_NUM; i ++)
        {
            ...
            if(socketinit)CH569NET_HandleSockInt(i,socketinit);                  /* 如果有中断则清零 */
        }
    }
}
...
#ifdef  KEEPLIVE_ENABLE
   CH569NET_SocketSetKeepLive( SocketId, 1 );                                   /* 开启socket的KEEPLIVE功能（V06版本支持） */
#endif

   i = CH569NET_SocketConnect(SocketId);                                        /* TCP连接 */
   mStopIfError(i);                                                             /* 检查错误 */
   i = CH569NET_SetSocketTTL( SocketId,100 );
   mStopIfError(i);                                                             /* 检查错误 */
}
#endif
...
void tcp_ip_stack_main_process(void)
{
    ...
    if(CH569NET_QueryGlobalInt())CH569NET_HandleGlobalInt();                /* 查询中断，如果有中断，则调用全局中断处理函数 */
}
