...
#define mem_local(x) ((unsigned int)x-0x20020000)/1024
...
/* 发送缓冲区地址 */
#define	pTx_Buff      (( UINT8 *)(ETH_queue_base+TxDscrOffset*TX_Des_Num+RxDscrOffset*RX_Des_Num))     /* 发送缓冲区 */
/* 接收缓冲区地址 */
#define	pRx_Buff      (( UINT8 *)(ETH_queue_base+TxDscrOffset*TX_Des_Num+RxDscrOffset*RX_Des_Num+/*1*/TX_Des_Num*TX_Buf_Size))     /* 接收缓冲区 */
...
#define  RX_BUF_IDLE                        0X00
#define  RX_BUF_ETH_LOCK                    0X01
#define  RX_BUF_STACK_LOCK                  0X02

#define  TX_BUF_IDLE                        0X00
#define  TX_BUF_ETH_LOCK                    0X01


typedef struct
{
    UINT32 RxbufStatus[RX_Des_Num]; //缓冲区状态  DMA只能使用处于idle的缓冲区,并把其锁定为ETH_LOCK
    UINT32 RxbufAddr[RX_Des_Num];   //缓冲区地址
    UINT32 RxbufLen[RX_Des_Num];    //接受长度
    UINT32 stack_index;
    UINT32 rev_index;
    UINT32 remaincount;
    UINT32 RxDmaEnable;
...
typedef struct
{
    //  ETH_DMADESCTypeDef *LastRxDes; /* 指向最近一次处理完的接收描述符的下一个描述符的地址，就是待处理的接收描述符的地址 */
        ETH_DMADESCTypeDef * RxDesReadNow;
