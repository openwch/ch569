 ...
 * @fn     Get_TxBuff_Addr
 *
 * @brief  Get current Tx queue Addr
 *
 * @return   current Tx queue Addr
 */
UINT8* Get_TxBuff_Addr(void)
{
    return ((UINT8*)(DMATxDescToSet->Buffer1Addr));
}
...
UINT8 ETH_buf_init(void)
{
	...
	Globe_RxDes_status.RxDesReadNow=(ETH_DMADESCTypeDef*)pDMARxDscrTab;/* 待处理的帧地址赋初值 */

	/* ltp */
	memset(&RXCTL,0,sizeof(RXCTL));
    RXCTL.RxDmaEnable = TRUE;
 ...
 * @return   None
 */
UINT8V  SendpktOK=0; /* 发送完成为0 ,mac_send置1 */
...
void ETH_IRQHandler(void)
{
    ...
    {
       if((DMARxDescToGet->Status&ETH_DMARxDesc_OWN)==0)/*判断描述符有没有读尽*/
       {
            ETH_DMAClearITPendingBit(ETH_DMA_IT_R);
            RXCTL.RxbufStatus[RXCTL.rev_index]=RX_BUF_STACK_LOCK;
            RXCTL.RxbufLen[RXCTL.rev_index]=((UINT16)(((DMARxDescToGet->Status)&0x3fff0000)>>16));
            RXCTL.RxbufAddr[RXCTL.rev_index]=(UINT32)(DMARxDescToGet->Buffer1Addr);
            Globe_RxDes_status.pengding_RxDes_cnt++;
            RXCTL.rev_index++;
            if(RXCTL.rev_index>=RX_Des_Num) RXCTL.rev_index=0;

            if(RXCTL.RxbufStatus[RXCTL.rev_index]==RX_BUF_IDLE)
            {
                DMARxDescToGet->Status|=ETH_DMARxDesc_OWN;//先释放当前的描述符
                DMARxDescToGet=(ETH_DMADESCTypeDef*)(DMARxDescToGet->Buffer2NextDescAddr);//当前的指向下一个
                RXCTL.RxDmaEnable = TRUE;
            }
            else /* 在处理本次的时候就停止下一次接收 */
            {
                DMARxDescToGet->Status|=ETH_DMARxDesc_OWN;        //先释放当前的描述符
                DMARxDescToGet=(ETH_DMADESCTypeDef*)(DMARxDescToGet->Buffer2NextDescAddr);//当前的指向下一个
                RXCTL.RxDmaEnable = TRUE;
            ...
            RXCTL.remaincount++;
            if( RXCTL.remaincount>3*RX_Des_Num) RXCTL.remaincount=RX_Des_Num;
       }
    }
    /* 清除DMASR标志位 */
 ...
 * @return   None
 */
void Enabl_MAC_RECV(void)
{
    ETH->DMAOMR |= ETH_DMAOMR_SR;           /* 开启接收DMA */
    ETH_DMAITConfig(ETH_DMA_IT_NIS
                    |ETH_DMA_IT_T       /* 使能发送完成中断 */
                    |ETH_DMA_IT_R       /* 使能接受DMA中断 */
                    ,ENABLE );//使能以太网发送中断
...
UINT16 mac_rece(UINT8 ** ptr)
{
    ...
    {
        *ptr=(UINT8*)(Globe_RxDes_status.RxDesReadNow->Buffer1Addr);
        frame_length=((UINT16)(((Globe_RxDes_status.RxDesReadNow->Status)&0x3fff0000)>>16));
...
void release_rx_des(void)
{
    Globe_RxDes_status.RxDesReadNow->Status |= ETH_DMARxDesc_OWN;/* 释放描述符 */
    (Globe_RxDes_status.RxDesReadNow) = (ETH_DMADESCTypeDef*)(Globe_RxDes_status.RxDesReadNow->Buffer2NextDescAddr);/* 当前在读描述符指针后移   */
    Globe_RxDes_status.pengding_RxDes_cnt--;/* 未读计数器自减  */
    /* 检测到发送流程停滞，则重启发送流程  */
